...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  ...
  sky += (0.1*streaks + 2.0*g*g*g + h*h*h)*vec3(0.0,0.5,2.0);
...
vec3 nlRenderGalaxy(vec3 vdir, vec3 fogColor, nl_environment env, float t) {
  ...
  vec3 stars = (0.8 + 0.2*sin(vec3(0.0,6.0,10.0)*(2.0*n1+0.8*n2) + vec3(0.0,0.4,0.82)))*st;
  ...
  stars += (0.4*gf + 0.012)*mix(vec3(0.0, 0.5, 0.85), gfcol*gfcol, NL_GALAXY_VIBRANCE);

  stars *= mix(1.0, NL_GALAXY_DAY_VISIBILITY, min(dot(fogColor, vec3(0.0,0.7,0.9)), 1.0)); // maybe add day factor to env for global use?
