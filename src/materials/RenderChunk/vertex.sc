...
#endif
$output v_color0, v_color1, v_fog, v_refl, v_texcoord0, v_lightmapUV, v_extra, v_worldPos, v_position,  v_color2, v_cloudPos 
...
void main() {
  ...
  #ifdef NL_GODRAY 
    //fogColor.a = mix(fogColor.a, 1.0, NL_GODRAY*nlRenderGodRayIntensity(cPos, worldPos, t, uv1, relativeDist, FogColor.rgb, FogAndDistanceControl.xy));
  #endif
  ...
  vec3 pre_cloudpos = a_position;
  vec3 cloudPos;
  pre_cloudpos.xz = pre_cloudpos.xz - 32.0;
  pre_cloudpos.y *= 0.01;
  cloudPos.x = pre_cloudpos.x*model[0][0];
  cloudPos.z = pre_cloudpos.z*model[2][2];
      #if BGFX_SHADER_LANGUAGE_GLSL
        cloudPos.y = pos.y+model[3][1];
      #else
        cloudPos.y = pos.y+model[1][3];
      #endif
